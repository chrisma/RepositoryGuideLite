...
The teams worked in a total of four sprints, the first three of which were to be done using the Scrum framework, while the last one was dedicated to Kanban.
...
Using the three dropdown menus at the top of the output, the user can change between sprints, teams, and the "time mode" (either hour of day or time of day) at will, which will automatically update the displayed heatmap with the new data, without the need to re-run the cell.
